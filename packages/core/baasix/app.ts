...
import { invalidateEntireCache, getCacheService } from "./services/CacheService.js";
...
import type { BaasixPlugin, PluginManagerOptions } from "./types/plugin.js";

// Class-based services for plugin context (instantiated per-request)
...
async function gracefulShutdown(signal: string) {
  ...
  try {
    // Shutdown plugins first (in reverse order)
    console.warn("Shutting down plugins...");
...
export async function startServer(options?: StartServerOptions | number) {
  ...
  const { plugins = [], pluginOptions = {} } = opts;
  ...
  const pluginManager = PluginManager.getInstance();
  pluginManager.setOptions({ verbose: pluginOptions.verbose ?? false });

  try {
    // Register plugins first (before database initialization)
    if (plugins.length > 0) {
      logger.info(`Registering ${plugins.length} plugin(s)...`);
      pluginManager.registerPlugins(plugins);

      // Get plugin schemas to register with schema manager
      const pluginSchemas = pluginManager.getPluginSchemas();
      if (pluginSchemas.length > 0) {
        logger.info(`Found ${pluginSchemas.length} plugin schema(s) to register`);
        schemaManager.registerPluginSchemas(pluginSchemas);
      }
    }
    ...
    // Initialize plugin services and routes after app initialization
    if (plugins.length > 0) {
      ...
      // Set plugin context with all available services
      pluginManager.setContext({
        // Database
        db,
        
        // Core service class (for instantiation)
        ItemsService,
        ...
        // Utility functions
        getCacheService,
        invalidateCache: invalidateEntireCache,
        
        // Class-based services (for per-request instantiation)
        FilesService,
        AssetsService,
        NotificationService,
        ReportService,
      ...
      await pluginManager.initializeServices();

      // Register plugin routes
      logger.info("Registering plugin routes...");
      pluginManager.registerRoutes(app, authMiddleware);

      // Register plugin hooks
      logger.info("Registering plugin hooks...");
      pluginManager.registerHooks(hooksManager);

      // Register plugin middleware
      pluginManager.registerMiddleware(app);

      // Call onInit for all plugins
      logger.info("Calling plugin onInit hooks...");
    ...
    server.listen(serverPort, async () => {
      ...
      // Call onReady for all plugins after server is listening
      if (plugins.length > 0) {
        logger.info("Calling plugin onReady hooks...");
        await pluginManager.readyPlugins();

        const summary = pluginManager.getSummary();
        logger.info(`✅ ${summary.total} plugin(s) loaded successfully`);
      }
    });
