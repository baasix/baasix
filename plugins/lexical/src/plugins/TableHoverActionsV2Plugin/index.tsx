...
function TableHoverActionsV2({
  ...
  useEffect(() => {
    ...
    const registerDropTarget = (cell: HTMLTableCellElement) =>
      dropTargetForElements({
        canDrop: ({source}: {source: any}) => isColumnDrag(source, tableKey),
        element: cell,
        getData: ({input}: {input: any}) => ({
          boundaryIndex: getBoundaryIndex(cell, input.clientX),
        }),
        onDrag: ({location, source}: {location: any; source: any}) => {
        ...
        onDragEnter: ({location, source}: {location: any; source: any}) => {
          if (!isColumnDrag(source, tableKey)) {
            return;
          }
          const boundaryIndex = getBoundaryIndex(
        ...
        onDrop: ({location, source}: {location: any; source: any}) => {
          setDropIndicatorState(null);
