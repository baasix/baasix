...
function DropDownItems({
  ...
  return (
    <DropDownContext.Provider value={contextValue}>
      <div className="dropdown" ref={dropDownRef as React.RefObject<HTMLDivElement>} onKeyDown={handleKeyDown}>
        {children}
      </div>
...
export default function DropDown({
  ...
  useEffect(() => {
    const handleButtonPositionUpdate = () => {
      if (showDropDown) {
        ...
        if (button !== null && dropDown !== null) {
          const {top, left} = button.getBoundingClientRect();
          const newTop = top / zoom + button.offsetHeight + dropDownPadding;
          if (newTop !== dropDown.getBoundingClientRect().top / zoom) {
            dropDown.style.top = `${newTop}px`;
          }
          const newLeft = Math.min(left, window.innerWidth - dropDown.offsetWidth - 20) / zoom;
          dropDown.style.left = `${newLeft}px`;
        }
      }
    };

    // Collect ancestor scrollable containers (e.g. Sheet/Dialog overlay)
    const scrollableAncestors: (HTMLElement | Document)[] = [document];
    ...
    for (const el of scrollableAncestors) {
      el.addEventListener('scroll', handleButtonPositionUpdate);
    }

    return () => {
      for (const el of scrollableAncestors) {
        el.removeEventListener('scroll', handleButtonPositionUpdate);
      }
    };
